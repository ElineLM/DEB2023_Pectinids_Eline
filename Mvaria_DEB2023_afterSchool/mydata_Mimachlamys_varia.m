...
metaData.data_0     = {'ab'; 'aj'; 'tp'; 'am'; 'Lb'; 'Lj'; 'Lp'; 'Li';'Ri';'GSI'};      
...
data.aj_nat = 15; units.aj_nat = 'd'; label.aj_nat = 'time since fertilisation at metam for natural environment'; bibkey.aj_nat = 'guess';
temp.aj_nat = C2K(14); units.temp.aj_nat = 'K'; label.temp.aj_nat = 'temperature';
comment.aj_nat = 'New guess to see if we can have other parameters from the f_Tinduff that is very low.';  
% in natural environment, the food quantity and quality might be higher
% (higher f value) and even if the temperature is lower the metamorphosis
% can occur before
...
data.Lj  = 0.0215 * 1.1451;   units.Lj  = 'cm';  label.Lj  = 'metamorphosis shell length';   bibkey.Lj  = 'TinduffHatchery';
  comment.Lj = 'Based on Tinduff hatchery protocol, with controlled temperature, conversion from length to height (from Regnier-Brisson L.)';
  temp.Lj = C2K(18); units.temp.Lj = 'K'; label.temp.Lj = 'temperature';
  
  
  data.Lj_nat = 0.03; units.Lj_nat = 'cm'; label.Lj_nat = 'metam shell length for natural environment'; bibkey.Lj_nat = 'guess';
temp.Lj_nat = C2K(14); units.temp.Lj_nat = 'K'; label.temp.Lj_nat = 'temperature';
comment.Lj_nat = 'New guess to see if we can have other parameters from the f_Tinduff that is very low.';  
% in natural environment, the food quantity and quality might be higher
% (higher f value) and even if the temperature is lower the metamorphosis
% can occur before with higher shell length
...
tLTinduff(:,2) = 1.1451 * tLTinduff(:,2); % if we want to convert the length into height
...
units.tLjuve   = {'d', 'cm'};  label.tLjuve = {'time', 'shell height'};  
...
%shell height, perpendicular to the hinge line
...
% 
% weights.tLlarvae        = 2 * weights.tLlarvae;
% weights.tLjuve          = 1 * weights.tLjuve; % no weight on juveniles, because we cannot find the way to have good predictions of larvae, juveniles and adults. 
% %keep the predictions for larvae, late juveniles and adults
weights.tL19SA        = 2 * weights.tL19SA;  
weights.tW19SA(1)       = 0 * weights.tW19SA(1); % for the first value that is repeated just to have the right dimension
% weights.tW19SA(2:end)   = 2 * weights.tW19SA(2:end); 
weights.LW19SA    = 2 * weights.LW19SA; 
% 
% weights.Lb = 2 * weights.Lb;
weights.Lj = 2 * weights.Lj;
...
% weights.aj_nat = 5 * weights.aj_nat;
