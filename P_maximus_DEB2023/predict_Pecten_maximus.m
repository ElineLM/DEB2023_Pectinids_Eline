...
%   TC_tL = tempcorr(temp.tL, T_ref, T_A);
  TC_aj = tempcorr(temp.aj, T_ref, T_A); %new data - JUNE 2023
  % zero-variate data
...
%   tT_j = (t_j - t_b)/ k_M/ TC_tj;   % d, time since birth at metam
  aT_j = t_j/ k_M/ TC_aj;           % d, time at metamorphosis, since fertilisation -- new data JUNE 2023
...
%   prdData.tj = tT_j;
  ...
  prdData.Wdi = Wd_i;
  prdData.Ri = RT_i;
  prdData.aj = aT_j; % new data -- JUNE 2023
...
%%% for new data -- JUNE 2023
%length-weight estimation /!\ E_R not considered
  EWd_Gal = d_V * ((LW_Galicia(:,1)*del_M ).^3  *(1 + f_Gal * w )) ; %in g, dry weight
% consider the shell height
...
%   prdData.tL = ELw;
  ...
  prdData.tL2 = ELw2;  
  prdData.tL3 = ELw3; 
  prdData.LW_Galicia = EWd_Gal;% new data -- JUNE 2023
