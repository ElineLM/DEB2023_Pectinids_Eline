...
data.aj = 29.5;    units.aj = 'd';    label.aj = 'time since fertilisation at metam'; bibkey.aj = 'TinduffHatchery';
  temp.aj = C2K(18);  units.temp.aj = 'K'; label.temp.aj = 'temperature';
  comment.aj = 'Based on Tinduff hatchery protocol, between 25-30 days plus 2 days of fixation, with controlled temperature';
 
data.Lb  = 0.0107;   units.Lb  = 'cm';  label.Lb  = 'shell length at birth';   bibkey.Lb  = 'TinduffHatchery';
  comment.Lb = 'Based on Tinduff hatchery protocol (between 105 to 110 µm), with controlled temperature';
  temp.Lb = C2K(19.5); units.temp.Lb = 'K'; label.temp.Lb = 'temperature';
data.Lj  = 0.0250;   units.Lj  = 'cm';  label.Lj  = 'metamorphosis shell length';   bibkey.Lj  = 'TinduffHatchery';
...
data.Lp  = 5;    units.Lp  = 'cm';  label.Lp  = 'shell height at puberty'; bibkey.Lp  = 'TinduffHatchery';
  comment.Lp = 'Estimation of first appearance of coral'; 
% 
% data.R_L  = 2*10000000/365; units.R_L  = '#/d'; label.R_L  = 'reprod rate at a large size';     bibkey.R_L  = 'TinduffHatchery';   
%    temp.R_L = C2K(14); units.temp.R_L = 'K'; label.temp.R_L = 'temperature';
%    comment.R_L = 'based on Tinduff Hatchery observations, for the individuals of 10.5 cm length, nb of eggs for one spawning event';
...
% temp.tLjuve    = C2K(18);  units.temp.tLjuve = 'K'; label.temp.tLjuve = 'temperature'; %18°C, temperature of the water at the Tinduff hatchery
...
dataSet_Galicia = [ ... % shell length (cm), shell height (cm), shell dry weight (g), somatic DW (g), gonad DW (g)
    ...
    %%% the standard deviation values are also available
...
units.LW_Galicia   = {'cm', 'cm', 'g', 'g', 'g'};  
label.LW_Galicia = {'shell length', 'shell height', 'shell dry weight', 'somatic dry weight', 'gonad dry weight'}; 
...
% %
% dataSet_Galicia_sd = [ ... % shell length (cm), shell height (cm), shell dry weight (g), somatic DW (g), gonad DW (g)
%     %%% standard deviations of several individuals, of class 1 for the age
%     %%% monitoring from April 1990 to July 1991. With one value
%     %%% approximately for each month
%     0.3     0.28	6.484	0.346	0.127
%     0.29	0.26	9.317	0.649	0.282
%     0.32	0.32	7.592	0.808	0.447
%     0.46	0.32	7.618	0.762	0.146
%     0.4     0.31	5.779	0.602	0.17
%     0.57	0.45	11.144	1.66	0.272
%     0.5     0.36	11.144	1.266	0.275
%     0.49	0.39	11.438	1.347	0.446
%     0.39	0.38	12.719	1.418	0.633
%     0.66	0.57	11.953	1.953	0.683
%     0.52	0.46	10.481	1.477	0.594
%     0.68	0.63	17.234	1.732	0.776
%     0.7     0.62	18.23	1.797	1.127
%     0.53	0.48	12.936	1.666	1.149
%     0.65	0.58	22.476	2.087	0.979
%     0.6     0.55	15.971	1.806	0.484
%     0.54	0.47	12.864	1.627	1.357
%     0.22	0.21	17.316	1.229	0.36
% ] ; 
...
% for new added data - June 2023
% weights.tLTinduff = 1 * weights.tLTinduff;
% weights.LW_Galicia = 1 * weights.LW_Galicia; 
...
'author = {Pazos, A.J. and Roman, G. and Aosta, C.P. and Abad, M. and Sanchez, J.L.}, ' ... 
'year = {1997}, ' ...
'title = {Seasonal changes in condition and biochemical composition of the scallop {P}ecten maximus L. from suspended culture in the Ria de Arousa (Galicia, N.W. Spain) in relation to environmental conditions}, ' ...
