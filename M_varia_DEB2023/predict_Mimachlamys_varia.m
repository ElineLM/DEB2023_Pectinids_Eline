...
%   TC_LW19SA = tempcorr(temp.LW19SA, T_ref, T_A);
  ...
  %%% filter for del_M
   if del_M_postmet >= 1 || del_M_larv >= 1
    info = 0; prdData = []; return;
   end

%    if f_tL19SA >= 1 || f_Tinduff >= 1
   if f_Tinduff >= 1
    info = 0; prdData = []; return;
   end
 ...
 % 
  ...
  Lw_j = L_j/ del_M_postmet;                % cm, shell height at metam at f
  ...
  Lw_p = L_p/ del_M_postmet;                % cm, shell height at puberty at f
  tT_p = (t_p - t_b)/ k_M/ TC_tp;   % d, time since birth at puberty at f and T
  
  % ultimate
  L_i = L_m * l_i;                  % cm, ultimate structural length at f
  Lw_i = L_i/ del_M_postmet  ;              % cm, ultimate total length at f
  ...
  W_at_LR = d_V * (L_R * del_M_postmet).^3  *(1 + f * w );  %in g, dry weight without gonad at f
...
%   
  ...
  Lw_j = l_j * L_m/ del_M_postmet;          % cm, physical length at metamorphosis
  Lw_i = l_i * L_m/ del_M_postmet;          % cm, physical ultimate length
...
%   init_cond_tLW = [L_metam*par.del_M_postmet, ((par.z*par.p_M/par.kap)/par.v)*(L_metam*par.del_M_postmet)^3, par.E_Hj, 0]; %[L_0, E_0, EH_0, ER_0] %try to start for metamorphosis time
...
%   E_H    = LEHR(:,3);             % J, maturity level
  ...
  Lw   = L/ del_M_postmet;          % cm, standard length
...
%   ELw_forW = Lw(2:end);
  EtWw_19SA = (ELw_19SA.*del_M_postmet).^3 + E_R .* w_E/ mu_E/ d_E + E.* w_E/ mu_E/ d_E;   % g ww, wet weight
  ...
  EWd_19SA = d_V * ((LW19SA(:,1)*del_M_postmet ).^3  *(1 + f_tL19SA * w )) ; %in g, dry weight
